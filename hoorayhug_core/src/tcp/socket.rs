...
use hoorayhug_syscall::socket2::Socket;
...
fn new_socket(addr: &SocketAddr, mptcp: bool) -> Result<Socket> {
    ...
    {
        let call = if mptcp {
            hoorayhug_syscall::new_mptcp_socket
        } else {
            hoorayhug_syscall::new_tcp_socket
        };
        call(addr)
    }
    ...
    {
        hoorayhug_syscall::new_tcp_socket(addr)
    }
}
...
pub fn bind(laddr: &SocketAddr, bind_opts: BindOpts) -> Result<TcpListener> {
    ...
    if let Some(iface) = bind_interface {
        hoorayhug_syscall::bind_to_device(&socket, &iface)?;
    }
...
pub(super) mod keepalive {
    use super::*;
    pub use hoorayhug_syscall::socket2::{SockRef, TcpKeepalive};
